...
public class BigBullet extends GameObject {
    ...
    public BigBullet(float x, float y, ObjectID ID, int velX, Handler handler,int type) {
        super(x, y, ID);
        this.velX = velX;
    ...
    public void render(Graphics graphic) {
        ...
        if(type ==1 )
         graphic.drawImage(texture.big_bullet_vec[0],(int)x,(int)y,null);
        if(type==2)
            graphic.drawImage(texture.big_bullet_vec[1],(int)x,(int)y,null);
    ...
    private void Colllison(LinkedList<GameObject>object) {
        ...
        boolean isSpawned = BigBullet.getSpawn();
        if(isSpawned) {
            for (int i = 0; i < handler.object.size(); i++) {
                ...
                if(tempObject.getID() == ObjectID.Eric_Boss)
                {
                    ...
                    {
                        handler.removeObject(this);
                        tempObject.decreaseHpBoss(2 * Player.damage_bullet);
                    }
                }
            }
        }
